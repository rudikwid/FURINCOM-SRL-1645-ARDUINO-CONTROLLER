...
//#include <Adafruit_SSD1306.h> //OLED ukuran 0.96, terlalu kecil
#include <Adafruit_SH1106.h>  //OLED ukuran 1.3"
...
#define freq_max        156000UL      // batas atas
...
//Adafruit_SSD1306 Layar(1);

Rotary DIAL = Rotary(dial_A, dial_B);

int dataPort[] = {8,9,10,11,12,A3,A2,13};
...
void display_settings()
{
   ...
   int r = map(db,50,550,5,126);                // pemetaan panjang bargraph dari nilai s-meter
  ...
  else
  {
     ...
     if (pake_Duplex) display.println(offsetTX); else display.println("Simplex");   //("direct");
  }
  display.display();
}
...
*/
void kirim_dataPLL(byte dataPLL) {
// lama waktu data = 0.3ms, lama waktu latch = 0.1ms

  for (byte kiriman = 0;kiriman<8;kiriman++){
      if (bitRead(dataPLL,kiriman)) digitalWrite(dataPort[kiriman],HIGH); 
      else digitalWrite(dataPort[kiriman],LOW);
}
...
void setup()
{
 // Serial.begin(115200);
  ...
  display.setTextSize(1);
  display.println("Furincom Control v2.1");
  display.println("");
  display.println("Juhar YC3TKM - YD0AYA");
  ...
  sei();                                     // panggil interrupt
  // Display first time  
  display_frequency();  // Update the display   
  wdt_enable(WDTO_2S);   //watch dog timer enable setelah 2 detik
}
...
//******************** eof -- end of file --- commisioned by rudik wid, 13 Okt 2022
//Perubahan 8-nop-2022
//Konversi ke OLED TFT SH1106 1.3"
//Konversi library Rotary ke rotary.h Ben Buxton 2011
